...
# %% Init Compute Data Science Algo on graph
db = driver.session()
...
def createGraphInDGDSCatalog(name = "mygraph"):
    results = db.run(
        "CALL gds.graph.create($graph_name ,'Website','LINKS_TO_tot')"
...
def createUndirectedGraphInGDSCatalog(name = "myundirectedgraph" ):
    results = db.run(
        "CALL gds.graph.create($graph_name, 'Website',{LINKS_TO_tot: {orientation: 'UNDIRECTED'}})"
        "YIELD graphName, nodeCount, relationshipCount, createMillis", {"graph_name":name}
        )
    for record in results:
...
computeAllCentralitiesAndCommunities("mygraph")

# %% sandbox test WWC_bij

def createUndirectedGraphInGDSCatalog(name = "myundirectedgraph" ):
    results = db.run(
        "CALL gds.graph.create($graph_name, 'Website',{LINKS_TO_tot_bij: {orientation: 'UNDIRECTED'}})"
        "YIELD graphName, nodeCount, relationshipCount, createMillis", {"graph_name":name}
        )
    for record in results:
        print("undirected graph created in GDS catalog with name:",record['graphName'],",",record['nodeCount'],"nodes,", record['relationshipCount'],"relationships in", record['createMillis'], "milliseconds")
...
createUndirectedGraphInGDSCatalog("undirgraphbij")

def computeWeaklyConnectedComponents(graph_name):
    result = db.run(
        "CALL gds.wcc.write($graph_name, {"
        "writeProperty: 'WCC_bij' })"
        "YIELD computeMillis, nodePropertiesWritten, componentCount"
        , {"graph_name":graph_name}
    )
    for record in result:
...
computeWeaklyConnectedComponents("undirgraphbij")

# %% sandbox pagerank for all nodes
# Using this tuto : https://towardsdatascience.com/how-to-use-cypher-projection-in-neo4j-graph-data-science-library-on-a-lord-of-the-rings-social-b3459138c4f1
...
result = db.run(
    "CALL gds.pageRank.write('whole-graph', {"
    "maxIterations: 100,"
    "dampingFactor: 0.85,"
    "writeProperty: 'PageRanktot'})"
    "YIELD nodePropertiesWritten, ranIterations, computeMillis, didConverge"
    )
for record in result:
    print("Unweighted PageRank run in", record["ranIterations"],"iterations and", record["computeMillis"], "ms on", record["nodePropertiesWritten"], "nodes and converged:",record["didConverge"])

# %% pagerank and Community for twitter
